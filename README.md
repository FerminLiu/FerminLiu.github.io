...
所以可见cv其实不需要这种操作，与 about.md 放在_pages 文件夹即可，但我还是为它单独搞了一个文件夹 _cv，不过我没有将 cv 各部分划分成若干子页面，而是把一个完整cv页面文件cv.md 放在了_cv 文件夹里。为了能让系统从文件夹 _cv 中读取 cv.md 文件，我们只需在_config.yml中collections中添加cv部分的参数即可。
# Getting Started
